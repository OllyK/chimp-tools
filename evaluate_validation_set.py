...
def train_detector(
    ...
    trainer._load_in_model_and_optimizer(0.0001, trainer.finetune_path)
    model_fn = f"{date.today()}_{settings.model_output_fn}.pytorch"
    model_out = Path(Path.cwd(), model_fn)
    # print(type(trainer), trainer.model_device, type(trainer.model), len(trainer.validation_loader.dataset))
    trainer.evaluate(trainer.validation_loader)
